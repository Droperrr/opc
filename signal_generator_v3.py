#!/usr/bin/env python3
"""
Модуль генерации сигналов v3.0 - Оптимизированная система с динамическими порогами
Расширенное покрытие всех торговых сессий и сезонные корректировки
...
class OptimizedSignalGenerator:
    ...
    def load_data(self):
        ...
        try:
            ...
            if os.path.exists('iv_aggregates_sample.csv'):
                self.iv_data = pd.read_csv('iv_aggregates_sample.csv', parse_dates=['timestamp'])
                logger.info(f"📈 Загружено {len(self.iv_data)} агрегированных записей")
            else:
                logger.warning("⚠️ Файл iv_aggregates_sample.csv не найден")
                self.iv_data = pd.DataFrame()
            
            # Загружаем трендовые сигналы
            if os.path.exists('trend_signals.csv'):
                self.trend_data = pd.read_csv('trend_signals.csv', parse_dates=['timestamp'])
                logger.info(f"📈 Загружено {len(self.trend_data)} трендовых сигналов")
            else:
                logger.warning("⚠️ Файл trend_signals.csv не найден")
                self.trend_data = pd.DataFrame()
            
            # Загружаем точки входа
            if os.path.exists('entry_points.csv'):
                self.entry_data = pd.read_csv('entry_points.csv', parse_dates=['timestamp'])
                logger.info(f"🎯 Загружено {len(self.entry_data)} точек входа")
            else:
                logger.warning("⚠️ Файл entry_points.csv не найден")
    ...
    def combine_data(self):
        """Объединяет все данные по временным меткам"""
        try:
    ...
    def save_optimized_signals(self, signals):
        ...
        try:
            if signals:
                ...
                df.to_csv('signals_optimized.csv', index=False)
                logger.info(f"💾 Сохранено {len(signals)} оптимизированных сигналов в signals_optimized.csv")
            else:
    ...
    def run(self):
        ...
        try:
            logger.info("🚀 Запуск оптимизированного генератора сигналов v3.0...")
            ...
            logger.info("✅ Оптимизированный генератор сигналов завершен успешно!")
