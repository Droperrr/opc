...
def get_order_book_data(symbol: str) -> Optional[Dict]:
    """
    Получение данных L1 order book (лучший bid/ask и объемы) из API Bybit.
    
    Args:
        symbol (str): Символ актива (например, SOLUSDT)
        
    Returns:
        Optional[Dict]: Словарь с данными order book или None в случае ошибки
    """
    try:
        ...
        logger.debug(f"📡 Запрос order book: {symbol}")
        ...
        if data['retCode'] != 0:
            logger.error(f"❌ API ошибка order book: {data['retMsg']}")
            return None
        
        if 'result' in data and 'list' in data['result'] and data['result']['list']:
            ticker_data = data['result']['list'][0]
            
            # Извлекаем лучшие bid и ask
            bid_price = float(ticker_data.get('bid1Price', 0))
            bid_volume = float(ticker_data.get('bid1Size', 0))
            ask_price = float(ticker_data.get('ask1Price', 0))
            ask_volume = float(ticker_data.get('ask1Size', 0))
            ...
            return {
                ...
                'timestamp': int(time.time() * 1000)  # Текущий timestamp в мс
            }
        
        logger.warning(f"⚠️ Нет данных order book для {symbol}")
        return None
        
    except requests.exceptions.RequestException as e:
        logger.error(f"❌ Ошибка запроса order book: {e}")
        return None
    except Exception as e:
        logger.error(f"❌ Неожиданная ошибка order book: {e}")
        return None
...
def load_data(df):
    ...
    """
    df.to_parquet('basis_raw_data_1m.parquet', index=False)
    print("\nData loaded to basis_raw_data_1m.parquet")
