...
def get_real_trades(currency, kind, start_timestamp, end_timestamp, count=1000):
    ...
    try:
        ...
        if trades:
            ...
            min_trade_time = min(trade['timestamp'] for trade in trades)
            ...
            print(f"Минимальное время сделки: {timestamp_to_datetime(min_trade_time)}")
            print(f"Запрошенный период: с {timestamp_to_datetime(start_timestamp)} по {timestamp_to_datetime(end_timestamp)}")
...
def main():
    ...
    end_timestamp = 1675209599999    # 2023-01-31 23:59:59.999
